    ...
    for (int t = 0; t < num_trials; t++) {
        ...
        q.submit([&](sycl::handler& h) {
            ...
            h.parallel_for<build_hashtable_s>(nd_range<1>(global,local),[=](nd_item<1> it){
                ...
                BlockPredEq<int,BLOCK_THREADS,ITEMS_PER_THREAD>(tid,items,flags,2,num_tile_items);
        ...
        q.submit([&](sycl::handler& h){
            ...
            h.parallel_for<build_hashtable_d>(nd_range<1>(global,local),[=](nd_item<1> it){
                ...
                BlockPredLTE<int,BLOCK_THREADS,ITEMS_PER_THREAD>(tid,items,flags,1997,num_tile_items);
            ...
            h.parallel_for<select_kernel>(nd_range<1>(global,local),[=](nd_item<1> it){
                ...
                for (int i = 0; i < ITEMS_PER_THREAD; ++i) {
                    if(flags[i] && (tid + BLOCK_THREADS * i < num_tile_items)){
                        int hash = (s_nation[i] * 25 * 7  + c_nation[i] * 7 +  (year[i] - 1992)) % ((1998-1992+1) * 25 * 25);
                        a_result[hash*5]= year[i];
                        a_result[hash*5+1]= c_nation[i];
                        a_result[hash*5+2]= s_nation[i];
                        sycl::atomic_ref<
                            unsigned long long,
                            sycl::memory_order::relaxed,
                            sycl::memory_scope::device,
                            sycl::access::address_space::global_space>
                            atomic_revenue(a_result[hash*5+4]);
        ...
        if(t>0) mean+=diff;
        {
            ...
            for (int i=0; i<res_size; i++) {
                if (host_res[5*i] != 0) {
                std::cout << host_res[5*i] << " " << host_res[5*i + 1] << " " << host_res[5*i + 2] << " " << reinterpret_cast<unsigned long long*>(&host_res[5*i + 4])[0]  << "\n";
                res_count += 1;
                }
            }
