    ...
    for (int t = 0; t < num_trials; t++) {
        ...
        q.submit([&](sycl::handler& h) {
            ...
            h.parallel_for<build_hashtable_s>(nd_range<1>(global,local),[=](nd_item<1> it){
                ...
                BlockLoad<decltype(a_s_nation),BLOCK_THREADS,ITEMS_PER_THREAD>(a_s_nation,tid,tile_offset,items2,num_tile_items);
        ...
        q.submit([&](sycl::handler& h){
            ...
            h.parallel_for<build_hashtable_p>(nd_range<1>(global,local),[=](nd_item<1> it){
                ...
                BlockLoad<decltype(a_p_category),BLOCK_THREADS,ITEMS_PER_THREAD>(a_p_category,tid,tile_offset,items2,num_tile_items);
        ...
        q.submit([&](sycl::handler& h){
            ...
            h.parallel_for<select_kernel>(nd_range<1>(global,local),[=](nd_item<1> it){
                ...
                BlockProbeAndPHT_2<decltype(a_p_hash_table),BLOCK_THREADS,ITEMS_PER_THREAD>(tid,items,category,flags,a_p_hash_table,P_LEN,num_tile_items);
                ...
                for (int i = 0; i < ITEMS_PER_THREAD; ++i) {
                    if(flags[i] && (tid + BLOCK_THREADS * i < num_tile_items)){
                        ...
                        a_result[hash*6]= year[i];
                        a_result[hash*6+1]= s_nation[i];
                        a_result[hash*6+2]= category[i];
                        sycl::atomic_ref<
                            unsigned long long,
                            sycl::memory_order::relaxed,
                            sycl::memory_scope::device,
                            sycl::access::address_space::global_space>
                            atomic_revenue(a_result[hash*6+4]);
