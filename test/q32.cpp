    ...
    for (int t = 0; t < num_trials; t++) {
        ...
        q.submit([&](sycl::handler& h){
            ...
            h.parallel_for<build_hashtable_c>(nd_range<1>(global,local),[=](nd_item<1> it){
                ...
                BlockLoad<decltype(a_c_nation),BLOCK_THREADS,ITEMS_PER_THREAD>(a_c_nation,tid,tile_offset,items,num_tile_items);
                BlockPredEq<int,BLOCK_THREADS,ITEMS_PER_THREAD>(tid,items,flags,24,num_tile_items);

                BlockLoad<decltype(a_c_custkey),BLOCK_THREADS,ITEMS_PER_THREAD>(a_c_custkey,tid,tile_offset,items,num_tile_items);
                BlockLoad<decltype(a_c_city),BLOCK_THREADS,ITEMS_PER_THREAD>(a_c_city,tid,tile_offset,items2,num_tile_items);
        ...
        q.submit([&](sycl::handler& h){
            ...
            h.parallel_for<build_hashtable_d>(nd_range<1>(global,local),[=](nd_item<1> it){
                ...
                BlockPredGTE<int,BLOCK_THREADS,ITEMS_PER_THREAD>(tid,items,flags,1992,num_tile_items);
                BlockPredALTE<int,BLOCK_THREADS,ITEMS_PER_THREAD>(tid,items,flags,1997,num_tile_items);
