    ...
    for (int t = 0; t < num_trials; t++) {
        ...
        q.submit([&](sycl::handler& h){
            ...
            h.parallel_for<build_hashtable_c>(nd_range<1>(global,local),[=](nd_item<1> it){
                ...
                BlockLoad<int,BLOCK_THREADS,ITEMS_PER_THREAD>(d_c_nation + tile_offset,tid,tile_offset,items,num_tile_items);
                BlockPredEq<int,BLOCK_THREADS,ITEMS_PER_THREAD>(tid,items,flags,24,num_tile_items);

                BlockLoad<int,BLOCK_THREADS,ITEMS_PER_THREAD>(d_c_custkey + tile_offset,tid,tile_offset,items,num_tile_items);
                BlockLoad<int,BLOCK_THREADS,ITEMS_PER_THREAD>(d_c_city + tile_offset,tid,tile_offset,items2,num_tile_items);
