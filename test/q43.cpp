    ...
    for (int t = 0; t < num_trials; t++) {
        ...
        q.submit([&](sycl::handler& h) {
            ...
            h.parallel_for<build_hashtable_s>(nd_range<1>(global,local),[=](nd_item<1> it){
                ...
                BlockLoad<int,BLOCK_THREADS,ITEMS_PER_THREAD>(d_s_nation + tile_offset,tid,tile_offset,items,num_tile_items);
                ...
                BlockLoad<int,BLOCK_THREADS,ITEMS_PER_THREAD>(d_s_city + tile_offset,tid,tile_offset,items2,num_tile_items);
        ...
        q.submit([&](sycl::handler& h){
            ...
            h.parallel_for<select_kernel>(nd_range<1>(global,local),[=](nd_item<1> it){
                ...
                BlockProbeAndPHT_2<int,int,BLOCK_THREADS,ITEMS_PER_THREAD>(tid,items,s_city,flags,d_s_hash_table,S_LEN,num_tile_items);
                ...
                for (int i = 0; i < ITEMS_PER_THREAD; ++i) {
                    if(flags[i] && (tid + BLOCK_THREADS * i < num_tile_items)){
                        ...
                        d_result[hash*4+1]= s_city[i];
                        d_result[hash*4+2]= brand[i];
                        sycl::atomic_ref<
                            unsigned long long,
