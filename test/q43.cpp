...
int main() {
    ...
    std::vector<int> h_s_nation = loadColumn<int>("s_nation", S_LEN);
    std::vector<int> h_s_city = loadColumn<int>("s_city", S_LEN);

    std::vector<int> h_p_partkey = loadColumn<int>("p_partkey", P_LEN);
    std::vector<int> h_p_category = loadColumn<int>("p_category", P_LEN);
    ...
    buffer<int> b_s_nation (h_s_nation.data(),range<1>(S_LEN));
    buffer<int> b_s_city (h_s_city.data(),range<1>(S_LEN));

    buffer<int> b_p_partkey (h_p_partkey.data(),range<1>(P_LEN));
    buffer<int> b_p_category (h_p_category.data(),range<1>(P_LEN));
    ...
    for (int t = 0; t < num_trials; t++) {
        ...
        q.submit([&](sycl::handler& h) {
            auto a_s_nation = b_s_nation.get_access<access_mode::read>(h);
            ...
            h.parallel_for<build_hashtable_s>(nd_range<1>(global,local),[=](nd_item<1> it){
                ...
                BlockLoad<decltype(a_s_nation),BLOCK_THREADS,ITEMS_PER_THREAD>(a_s_nation,tid,tile_offset,items,num_tile_items);
                BlockPredEq<int,BLOCK_THREADS,ITEMS_PER_THREAD>(tid,items,flags,24,num_tile_items);

                BlockLoad<decltype(a_s_suppkey),BLOCK_THREADS,ITEMS_PER_THREAD>(a_s_suppkey,tid,tile_offset,items,num_tile_items);
                BlockLoad<decltype(a_s_city),BLOCK_THREADS,ITEMS_PER_THREAD>(a_s_city,tid,tile_offset,items2,num_tile_items);
                BlockBuildSelectivePHT_2<decltype(a_s_hash_table),BLOCK_THREADS,ITEMS_PER_THREAD>(tid,items,items2,flags,a_s_hash_table,S_LEN,num_tile_items);
            });
        });
        ...
        q.submit([&](sycl::handler& h){
            ...
            h.parallel_for<build_hashtable_c>(nd_range<1>(global,local),[=](nd_item<1> it){
                ...
                BlockBuildSelectivePHT_1<decltype(a_c_hash_table),BLOCK_THREADS,ITEMS_PER_THREAD>(tid,items,flags,a_c_hash_table,C_LEN,num_tile_items);
            });
        });
        
        // 3 build_hashtable_p
        q.submit([&](sycl::handler& h){
            auto a_p_category = b_p_category.get_access<access_mode::read>(h);
            ...
            h.parallel_for<build_hashtable_p>(nd_range<1>(global,local),[=](nd_item<1> it){
                ...
                BlockLoad<decltype(a_p_category),BLOCK_THREADS,ITEMS_PER_THREAD>(a_p_category,tid,tile_offset,items,num_tile_items);
                ...
                BlockLoad<decltype(a_p_brand1),BLOCK_THREADS,ITEMS_PER_THREAD>(a_p_brand1,tid,tile_offset,items2,num_tile_items);
                BlockBuildSelectivePHT_2<decltype(a_p_hash_table),BLOCK_THREADS,ITEMS_PER_THREAD>(tid,items,items2,flags,a_p_hash_table,P_LEN,num_tile_items);
            });
        });
        ...
        q.submit([&](sycl::handler& h){
            ...
            h.parallel_for<build_hashtable_d>(nd_range<1>(global,local),[=](nd_item<1> it){
                ...
                BlockLoad<decltype(a_d_year),BLOCK_THREADS,ITEMS_PER_THREAD>(a_d_year,tid,tile_offset,items,num_tile_items);
                ...
                BlockLoad<decltype(a_d_datekey),BLOCK_THREADS,ITEMS_PER_THREAD>(a_d_datekey,tid,tile_offset,items2,num_tile_items);
                BlockBuildSelectivePHT_2<decltype(a_d_hash_table),BLOCK_THREADS,ITEMS_PER_THREAD>(tid,items2,items,flags,a_d_hash_table,d_val_len,19920101,num_tile_items);
            });
        });
        ...
        q.submit([&](sycl::handler& h){
            ...
            h.parallel_for<select_kernel>(nd_range<1>(global,local),[=](nd_item<1> it){
                ...
                BlockProbeAndPHT_2<decltype(a_s_hash_table),BLOCK_THREADS,ITEMS_PER_THREAD>(tid,items,s_city,flags,a_s_hash_table,S_LEN,num_tile_items);

                BlockLoad<decltype(a_lo_custkey),BLOCK_THREADS,ITEMS_PER_THREAD>(a_lo_custkey,tid,tile_offset,items,num_tile_items);
                BlockProbeAndPHT_1<decltype(a_c_hash_table),BLOCK_THREADS,ITEMS_PER_THREAD>(tid,items,flags,a_c_hash_table,C_LEN,num_tile_items);
                ...
                for (int i = 0; i < ITEMS_PER_THREAD; ++i) {
                    if(flags[i] && (tid + BLOCK_THREADS * i < num_tile_items)){
                        int hash = ((year[i] - 1992) * 250 * 1000 + s_city[i] * 1000 + brand[i]) % ((1998-1992+1) * 250 * 1000);
                        a_result[hash*4]= year[i];
                        a_result[hash*4+1]= s_city[i];
                        a_result[hash*4+2]= brand[i];
                        sycl::atomic_ref<
                            unsigned long long,
