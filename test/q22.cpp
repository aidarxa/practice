...
int main() {
    ...
    auto v_p_brand1 = loadColumn<int>("p_brand1", P_LEN);
    int* h_p_brand1 = malloc_host<int>(P_LEN, q);
    std::copy(v_p_brand1.begin(), v_p_brand1.end(), h_p_brand1);
    int* d_p_brand1 = malloc_device<int>(P_LEN, q);
    q.memcpy(d_p_brand1, h_p_brand1, P_LEN * sizeof(int));
    ...
    for (int t = 0; t < num_trials; t++) {
        ...
        q.submit([&](sycl::handler& h){
            ...
            h.parallel_for<build_hashtable_p>(nd_range<1>(global,local),[=](nd_item<1> it){
                ...
                BlockLoad<int,BLOCK_THREADS,ITEMS_PER_THREAD>(d_p_brand1 + tile_offset,tid,tile_offset,items,num_tile_items);
                BlockPredGTE<int,BLOCK_THREADS,ITEMS_PER_THREAD>(tid,items,flags,260,num_tile_items);
                BlockPredALTE<int,BLOCK_THREADS,ITEMS_PER_THREAD>(tid,items,flags,267,num_tile_items);

                BlockLoad<int,BLOCK_THREADS,ITEMS_PER_THREAD>(d_p_partkey + tile_offset,tid,tile_offset,items2,num_tile_items);
                BlockBuildSelectivePHT_2<int,int,BLOCK_THREADS,ITEMS_PER_THREAD>(tid,items2,items,flags,d_p_hash_table,P_LEN,num_tile_items);
            });
        });
        // 3 build_hashtable_d
        q.submit([&](sycl::handler& h){
        ...
        q.submit([&](sycl::handler& h){
            ...
            h.parallel_for<select_kernel>(nd_range<1>(global,local),[=](nd_item<1> it){
                ...
                BlockProbeAndPHT_2<int,int,BLOCK_THREADS,ITEMS_PER_THREAD>(tid,items,brand,flags,d_p_hash_table,P_LEN,num_tile_items);
                ...
                for (int i = 0; i < ITEMS_PER_THREAD; ++i) {
                    if(flags[i] && (tid + BLOCK_THREADS * i < num_tile_items)){
                        ...
                        d_result[hash*3]= year[i];
                        d_result[hash*3+1]= brand[i];
                        sycl::atomic_ref<
                            unsigned long long,
                            sycl::memory_order::relaxed,
                            sycl::memory_scope::device,
                            sycl::access::address_space::global_space>
                            atomic_revenue(d_result[hash*3+2]);
