...
- SQL-level `ORDER BY` and `LIMIT` on final GPU materialized results. Bounded small limits use an exact GPU Top-K selection path; single-key numeric `ORDER BY ... DESC LIMIT K` can use adaptive threshold Top-K for larger `K` when it is substantially smaller than the input. Simple scan/projection Top-N can use a direct radix path before full dense materialization. Unbounded ordering and very large limits fall back to device-side Bitonic Sort.
...
1. Broader projection-level Top-K pushdown for joins, filters, nullable keys, multi-key ordering, and ascending order. The simple scan/projection single-key numeric descending path is current.
